...
namespace brain{
     ...
     * @param f_steeringControl_camera     reference to steering motor control interface for camera
    ...
    {   
        switch(m_state)
        {
            ...
            case 1:
                ...
                m_steeringControl_camera.setAngle(m_angle); // control the steering angle  for camera
    ...
    void CRobotStateMachine::serialCallbackMove(char const * a, char * b)
    {
        ...
        if (2 == l_res)
        {
            ...
            if( !m_steeringControl.inRange(l_angle) || m_steeringControl_camera.inRange(l_angle)){ // Check the received steering angle
    ...
    void CRobotStateMachine::serialCallbackBrake(char const * a, char * b)
    {
        ...
        if(1 == l_res)
        {
            if( !m_steeringControl.inRange(l_angle) || !m_steeringControl_camera.inRange(l_angle)){
                sprintf(b,"The steering angle command is too high;;");
                return;
            }
            m_speed = 0;
